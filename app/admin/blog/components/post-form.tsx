...
export default function PostForm({ 
  ...
  showSuccess: showSuccessProp = false,
  ...
  const [showSuccess, setShowSuccess] = useState(showSuccessProp);
  const [localIsSubmitting, setLocalIsSubmitting] = useState(false);
  
  // Sync isSubmitting state with prop
  useEffect(() => {
    console.log('isSubmitting prop changed:', submittingProp);
    setIsSubmitting(submittingProp);
  }, [submittingProp]);
  
  // Sync showSuccess state with prop
  useEffect(() => {
    console.log('showSuccess prop changed:', showSuccessProp);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (localIsSubmitting) return; // Prevent multiple submissions
    
    try {
      // Set both local and parent state to true
      setLocalIsSubmitting(true);
      setIsSubmitting(true);
      
      // Also update parent component state if available
      if (setIsSubmittingProp) {
        setIsSubmittingProp(true);
      }
      
      console.log('PostForm: Starting form submission');
      ...
      console.log('PostForm: onSubmit completed');
    } catch (error) {
      // Show error toast
      ...
      setLocalIsSubmitting(false);
      console.log('PostForm: Reset localIsSubmitting state');
    }
  };
